...
|------|-------|-------|-------|-------|
| **입력** | 05_arch_architecture.md + 01_input_data.json + brainstorm + research_deep | 05_arch_lesson_plan.md + 01_input_data.json + brainstorm + research_deep + 06_write_lecture_outline.md | 05_arch_slide_structure.md + 01_input_data.json + brainstorm + 06_sessions/ | 01_input_data.json + 06_write_slide_plan.md |
...
## 강의교안 작성 — 참조 라이브러리 (Phase 6a~6d, 7a에서 참조)

> **주의**: 이 섹션은 **실행 흐름을 정의하지 않습니다**. 실행 흐름은 SKILL.md의 Phase 6 오케스트레이션 루프와 아래의 "강의교안 차시별 작성 (Phase 6a)" 이하 섹션을 따릅니다.
> 이 섹션은 Phase 6a(session_write), 6c(module_integrate), 7a(final_integrate)에서 공통으로 참조하는 **교수 모델별 구조, 스크립트 밀도 테이블, 발문/형성평가 배치 규칙, 검증 기준**을 정의합니다.

교안 Phase 6은 교수자가 수업에서 직접 사용할 수 있는 상세 교안 스크립트를 생성한다. 구성안 Phase 6이 "코스 전체를 어떻게 구성할까"(코스 레벨)를 작성했다면, 교안 Phase 6은 **"교수자가 무엇을 말하고, 어떤 질문을 던지고, 어떤 활동을 진행하는가"**(레슨 레벨)를 스크립트 수준으로 구체화한다.

### 이 섹션의 구성

| 하위 섹션 | 참조하는 실행 단계 |
|-----------|------------------|
| Step 0: 조건부 분기 결정 | Phase 6a session_write 작성 절차 Step 1 |
| Step 2: 교수 모델별 구조 | Phase 6a session_write 작성 절차 Step 3~5 |
| Step 3: 검증 기준 | Phase 6c module_integrate 자체검증, Phase 7a final_integrate |
| Step 4: 부록·매트릭스 형식 | Phase 7a final_integrate 통합 절차 Step 4 |
| 수정 모드 | Phase 7b 이후 REVISE 분기 시 |

### Step 0: 컨텍스트 로드 + 조건부 분기 결정 [참조]
...
### Step 2: 차시별 스크립트 구성 요소 [참조 — 교수 모델별 구조, 밀도 테이블]
...
### Step 4: 부록·매트릭스 형식 [참조 — 7a 통합 시 적용]
...
### 수정 모드 (Phase 7b REVISE 판정 시) [실행 — SKILL.md Phase 7b 이후 분기에서 호출]
...
- 조건부 분기(script_detail_level, gagne_display, questioning_design, formative_assessment, teaching_model)는 위 "참조 라이브러리" 섹션의 Step 0(동작 2~4) 및 Step 2(동작 2~5)의 규칙을 동일하게 적용
...
**설계 근거**: 교안 Phase 6(차시별 작성)의 3계층 아키텍처(Micro-Meso-Macro)를 슬라이드 기획에 적응. 순차 작성, Visual Summary(Running Summary 적응), 경량 자체 검증, Day 통합, 최종 통합 패턴을 슬라이드 특성에 맞게 변환.

### 3계층 아키텍처

```
05_arch_slide_structure.md + 01_input_data.json + 03_brainstorm_result.md
+ {script_dir}/06_sessions/ (교안 세션 파일)
        │
        ▼
  ┌─────────────┐
  │ Step 0      │ 컨텍스트 로드 + 작성 전략 수립
  │ Read, Write │ 구조 설계, 시각화 전략, Visual Summary 초기화, 분할 모드 결정
  └──────┬──────┘
         │
         ▼
  ┌─────────────────────────────────────────────┐
  │ Micro: Step 1 — 세션별 슬라이드 기획 (session_plan 모드)  │
  │ Read, Write  │ 1세션씩 순차 작성 → 자체 검증 4항목 → Visual Summary 갱신  │
  │              │ [세션 수만큼 반복]                                         │
  └──────┬──────┘
         │ (Day 경계 도달 시)
         ▼
  ┌─────────────────────────────────────────────┐
  │ Meso: Step 2 — Day 통합 (day_integrate 모드)             │
  │ Read, Write  │ Day 내 세션 병합 + 시각 패턴 패치 + 전환 연결              │
  │              │ [Day 수만큼 반복] (≤10세션 시 생략 → 크로스-세션 검증만)    │
  └──────┬──────┘
         │
         ▼
  ┌─────────────────────────────────────────────┐
  │ Macro: Step 3 — 최종 통합 (final_integrate 모드)         │
  │ Read, Write  │ 전체 병합 + 코스 레벨 섹션 + SLO 매핑 매트릭스 + 6중 검증  │
  └─────────────┘
```
...
```
{output_dir}/
├── _visual_summary.md                # Visual Summary (작성 중 갱신)
├── 06_write_slide_plan_draft.md      # Step 1-2 중간산출물
...
{output_dir}/
├── _visual_summary.md                # Visual Summary (작성 중 갱신)
├── 06_session_plans/                 # Step 1: 세션별 기획
│   ├── 06_session_plan_001.md
│   ├── 06_session_plan_002.md
│   └── ...
├── 06_day_plans/                     # Step 2: Day 통합
│   ├── 06_day_plan_01.md
│   └── ...
└── 06_write_slide_plan.md            # Step 3 최종 ★
```
...
| 총 세션 수 | 모드 | Micro (Step 1) | Meso (Step 2) | Macro (Step 3) |
|-----------|------|--------------|-------------|-------------|
| **≤10** | **단일** | 세션별 순차 작성 (단일 draft 파일 내 섹션) | **생략** — 크로스-세션 일관성 검증만 | 최종 정제 + 6중 검증 |
| **11-20** | **2계층** | 세션별 순차 작성 (단일 draft 파일) | Day 통합 (draft 파일 내 Day 섹션) | 최종 정제 + 6중 검증 |
| **21+** | **3계층 full** | 세션별 파일 생성 (`06_session_plans/`) | Day별 파일 생성 (`06_day_plans/`) | 전체 병합 + 코스 레벨 섹션 |
...
| 항목 | 내용 |
|------|------|
| 입력 | `05_arch_slide_structure.md`, `01_input_data.json`, `03_brainstorm_result.md`, `{script_dir}/06_sessions/` |
| 도구 | Read, Write |
| 산출물 | 내부 컨텍스트 + `_visual_summary.md` 초기 생성 |

#### 동작

1. **05_arch_slide_structure.md 전체 로드**
   - §2 세션별 슬라이드 시퀀스 (유형, 순서, 시간, SLO 매핑, 레이아웃, Gagné)
   - §4 검증 결과 확인 (6중 검증 통과 여부)
   - §1 세션 유형 분류 (concept/code/practice/project/mixed)
...
3. **분할 모드 결정**
   - `total_sessions` 기반: ≤10 단일 / 11-20 2계층 / 21+ 3계층 full
   - 3계층 full 시 `06_session_plans/`, `06_day_plans/` 디렉토리 생성

4. **Visual Summary 초기화** — `_visual_summary.md` 생성
   ```markdown
   # Visual Summary
   > 세션별 시각 패턴 누적 추적. Step 1 작성 시 자동 갱신, Step 2 Day 통합 시 리셋.

   ## 글로벌 설정
   - 도구: {tool.label}
   - 디자인 톤: {design_tone.label}
   - 코드 테마: {code_theme.theme}
   - A-E 수준: {assertion_evidence.level}
   ```
...
### Step 1: 세션별 슬라이드 기획 작성 — `session_plan` 모드

교안 Phase 6a(session_write)에 대응. 1세션씩 순차 작성하며, 직전 세션의 마지막 슬라이드를 참조하여 내러티브를 연결한다.

#### 입력

| 파일 | 용도 | 교안 대응 |
|------|------|---------|
| `05_arch_slide_structure.md` §2 해당 세션 | 슬라이드 시퀀스, 유형, 시간, SLO, 레이아웃, Gagné | `05_arch_lesson_plan.md` 해당 차시 |
| `{script_dir}/06_sessions/06_session_{NNN}.md` | 교안 콘텐츠 — 도입/전개/정리, 발문, 활동, 형성평가 | (교안 자체가 입력원) |
| `03_brainstorm_result.md` 관련 섹션 | 시각화 전략, 레이아웃 권장, 메타포 매핑 | 발문/활동/사례 후보 |
| `01_input_data.json` | slide_settings, 도구 제약, 밀도 기준, design_tone | script_settings |
| `_visual_summary.md` | 이전 세션 누적 시각 패턴 | `_running_summary.md` |
| 직전 세션 기획 (마지막 2-3 슬라이드) | Assertion 내러티브 연결, 전환 슬라이드 연결 | `06_session_{N-1}.md` |

#### 산출물

- **≤20세션**: `06_write_slide_plan_draft.md` 내 해당 세션 섹션 추가(append)
- **21+세션**: `06_session_plans/06_session_plan_{NNN}.md` 개별 파일

#### 작성 절차 (7단계)

각 세션에 대해 순차적으로 (세션 1 → 세션 2 → ... → 세션 N):

**1. 컨텍스트 로드**
   - arch §2에서 해당 세션의 유형, 슬라이드 수, SLO, Gagné 매핑, 시간 배분 추출
   - 해당 교안 파일 Read — `06_sessions/06_session_{NNN}.md`
   - brainstorm §3에서 해당 세션의 시각화 계획 추출
   - `_visual_summary.md` 마지막 세션 확인 — 레이아웃 빈도, Assertion 톤, 전환 상태

**2. 도입 슬라이드 기획** (2-3장: title + agenda + 선수지식 확인)
   - Gagné 사태 1(주의 획득), 2(목표 고지), 3(선수지식 자극) 매핑
   - 교안 도입부의 Hook, SLO 제시, Pre-assessment 참조
   - title: 강의 제목 + 세션 번호 (Assertion 불필요)
   - agenda: 차시 학습 목표 + 흐름 안내 (Assertion 불필요)
   - (조건부) 선수지식 확인 quiz: `has_quiz_content = true` 시

**3. 본론 슬라이드 기획** (유형별 상세, 세션의 핵심)
   - arch §2의 슬라이드 시퀀스를 따름 — 유형, 순서, 시간 준수
   - 각 슬라이드별 9필드 작성 (아래 "슬라이드 상세 필드" 참조)
   - Gagné 사태 4~8 매핑
   - brainstorm §2(유형별 시각화) + §5(코드 전략, 조건부) 참조
   - 교안 전개부의 핵심 설명, 활동, 형성평가에서 콘텐츠 추출
   - code 유형 슬라이드 시 Context7 조회 (조건부, 아래 프로토콜 참조)
   - **1 아이디어/1 슬라이드 원칙** 엄수 — 핵심 시각/텍스트 요소 ≤3 (Rule of Three)

**4. 마무리 슬라이드 기획** (2-3장: summary + Exit Ticket + 예고)
   - Gagné 사태 9(파지와 전이 촉진) 매핑
   - 교안 정리부의 요약, 형성평가, 차시 예고 참조
   - summary: 핵심 3가지 요약 (A-E 변형)
   - (조건부) quiz: `formative_assessment.type ≠ none` 시 Exit Ticket 슬라이드
   - 다음 세션 예고: 전환 멘트와 연결

**5. 발표자 노트 보강** — 5항목 구조
   - 모든 슬라이드의 발표자 노트 완성 확인
   - 전환 멘트가 직전·직후 슬라이드와 자연스럽게 연결되는지 확인
   - 타이밍 큐 추가 (아래 규칙 참조)

**6. 자체 검증** — 4항목 즉시 검증 (아래 "세션별 자체 검증" 참조)
   - FAIL 항목 발생 시 해당 슬라이드만 수정 1회

**7. Visual Summary 갱신** — `_visual_summary.md`에 세션 요약 append

#### 슬라이드 상세 필드 (각 슬라이드마다)

| 필드 | 설명 | 작성 가이드 |
|------|------|-----------|
| **유형** | 13가지 중 선택 | 05_arch §2에서 결정됨 |
| **Assertion Headline** | 완전한 주장 문장 | A-E partial: "~이다", "~한다" 형식. 아래 A-E 규칙 참조 |
| **콘텐츠 구성** | 핵심 내용 요소 | 불릿 3-5개, 교안 해당 섹션에서 추출. 핵심 요소 ≤3 (Rule of Three) |
| **시각 자료** | 시각 증거 설명 | brainstorm §2 시각화 전략 반영, 구체적 시각물 기술 (30-40% 시각 증거) |
| **레이아웃** | 배치 패턴 | 도구 제약 내, arch §6 도구별 레이아웃 매핑 참조 |
| **발표자 노트** | 교수자 지원 정보 | 5항목 (핵심포인트, 보충설명, 전환멘트, 교수자행동, 타이밍큐) |
| **시간** | 분 | 유형별 범위 내, arch §2에서 배정된 시간 |
| **SLO** | 관련 학습 목표 | arch §2에서 매핑된 SLO ID |
| **교안 매핑** | 교안 섹션 참조 | 어느 세션의 어느 단계(도입/전개/정리)에서 파생되었는지 |

#### Assertion Headline 작성 규칙

**A-E 적용 유형** (partial: Headline은 주장 문장, 시각 증거 30-40%):

| 유형 | A-E 적용 | Headline 형식 | 예시 |
|------|---------|-------------|------|
| concept | ✅ partial | 주장 문장 — "~이다", "~한다" | "머신러닝은 데이터에서 패턴을 학습하여 예측하는 알고리즘이다" |
| comparison | ✅ partial | 비교 결론 — "A가 B보다 ~하다" | "CNN이 RNN보다 이미지 분류에 적합하다" |
| data_insight | ✅ partial | 데이터 해석 — "~를 보여준다" | "학습률 0.01이 가장 빠른 수렴을 보여준다" |
| code | 변형 | 핵심 원리 — "~로 구현한다" | "리스트 컴프리헨션으로 데이터를 한 줄로 변환한다" |
| summary | 변형 | 핵심 메시지 — "~가 핵심이다" | "환경 설정의 핵심은 재현 가능성이다" |
| timeline | 변형 | 시간적 주장 — "~에서 ~로 발전했다" | "2012년 AlexNet에서 2024년 GPT-4o로 발전했다" |
| image | ✅ partial | 시각 해석 — "~를 보여준다" | "이 다이어그램은 신경망의 순전파 과정을 보여준다" |
| activity | 변형 | 활동 목표 — "~를 실습한다" | "직접 모델을 학습시키고 결과를 비교한다" |
| title | — | 일반 제목 | "Day 1: 딥러닝 기초" |
| agenda | — | 일반 제목 | "이번 시간 학습 목표" |
| section_transition | — | 전환 문구 | "핵심 개념을 확인했으니, 이제 실습으로 넘어갑니다" |
| quiz | — | 질문 형태 | "다음 중 올바른 설명은?" |
| quote | — | 인용문 | "데이터는 새로운 석유이다 — Clive Humby" |
...
| # | 항목 | 내용 | 예시 |
|---|------|------|------|
| 1 | **핵심 포인트** | 이 슬라이드에서 반드시 전달할 메시지 (1-2문장) | "이 슬라이드의 핵심: 오버피팅은 학습 데이터에만 잘 맞는 현상" |
| 2 | **보충 설명** | 슬라이드에 없지만 구두로 보충할 내용 | "실무에서는 교차검증으로 오버피팅을 탐지합니다" |
| 3 | **전환 멘트** | 다음 슬라이드로 넘어갈 때 사용할 연결 문장 | "오버피팅을 이해했으니, 이를 해결하는 정규화 기법을 살펴보겠습니다" |
| 4 | **교수자 행동** | 시연, 질문 던지기, 대기, 화면 전환 등 물리적 행동 | "[질문] 학습자에게 2초 대기 → 손들기 → 지명 답변" |
| 5 | **타이밍 큐** | 이 슬라이드 소요 시간 + 누적 시간 + 페이싱 가이드 | "[4분/누적 12분] 설명 2분 + 질문 1분 + 대기 1분" |

> **전환 멘트 유형**: (1) 개념 연결 — "A를 이해했으니 B를 봅시다", (2) 질문 전환 — "그렇다면 ~는 어떨까요?", (3) 시각 전환 — "이제 실제 코드로 확인해 보겠습니다"

#### 세션별 자체 검증 (4항목)

각 세션 기획 완료 후 즉시 수행. 교안 Phase 6b(session_review)에 대응하되, writer-agent 자체 검증으로 수행 (Phase 5에서 review-agent가 독립 검토).

| # | 검증 항목 | 기준 | FAIL 시 처리 |
|---|---------|------|------------|
| V1 | **밀도 범위** | 각 슬라이드의 콘텐츠 항목 수가 유형별 밀도 범위(줄) 이내. 핵심 시각/텍스트 요소 ≤3 | 항목 추가/삭제로 범위 내 조정 |
| V2 | **시간 합산** | 슬라이드별 시간 합 ≈ session_minutes - activity_time (±3분 허용) | 시간 재배분 (유연한 유형에서 조정) |
| V3 | **SLO 커버리지** | 해당 세션의 대상 SLO 전체가 최소 1개 슬라이드에 매핑 | 미커버 SLO에 슬라이드 추가/기존 슬라이드 SLO 매핑 |
| V4 | **Assertion 품질** | concept/comparison/data_insight/image 유형의 Headline이 완전한 주장 문장 (키워드·질문 금지) | Headline 재작성 |

**재시도 규칙**: FAIL → 해당 슬라이드만 수정 1회 → 2차 FAIL → 경고 기록(`[WARN] 세션 {N} V{X} 2차 FAIL`) 후 진행 (Step 2 Day 통합 또는 Step 3에서 처리)

#### Context7 조회 프로토콜 (조건부)

> 코드 슬라이드의 코드 예제·API 시그니처 정확성을 확보하기 위해 Context7 MCP를 직접 호출한다.

**활성화 조건**: `has_code_content = true` AND 현재 세션이 `code` 유형 슬라이드 포함

**호출 절차**:
1. `resolve-library-id(libraryName="{기술 키워드}")` → 최적 라이브러리 ID 선택
2. `get-library-docs(context7CompatibleLibraryID="{ID}", topic="{해당 코드 슬라이드 주제}", tokens=3000)` → 공식 코드 예제 수신
3. 슬라이드 반영: 콘텐츠 구성의 코드 블록 + 발표자 노트에 출처 기록 `[C7] {라이브러리명} 공식 문서 — {주제}`

**제한**:
- **세션당 최대 1회** (resolve + get-docs를 1회로 카운트)
- Phase 2 브레인스토밍, Phase 3 구조 설계에서 이미 수집된 코드 정보 우선 사용
- 교수법·시각화·레이아웃 관련에는 Context7 사용 금지 (기술 코드/API 전용)

#### Visual Summary 갱신 규칙

각 세션 기획 완료 후 `_visual_summary.md`에 append. 교안 Running Summary에 대응하되, 시각 패턴 추적으로 전환.

**5항목**:

| # | 항목 | 내용 | 용도 |
|---|------|------|------|
| 1 | 레이아웃 빈도 | 이 세션에서 사용한 레이아웃 패턴 목록 | 후속 세션에서 과도한 반복 방지 |
| 2 | 유형 분포 | 사용된 슬라이드 유형 카운트 | 유형 다양성 모니터링 |
| 3 | Assertion 톤 | 마지막 3개 Assertion Headline | 톤 일관성 + 내러티브 연속성 |
| 4 | 시각 요소 | 사용된 핵심 시각 요소 (다이어그램, 아이콘, 색상 강조) | 시각 패턴 일관성 |
| 5 | 전환 상태 | 마지막 슬라이드의 전환 멘트/예고 | 다음 세션 첫 슬라이드 연결 |

**형식**:
```markdown
### 세션 {N}: {제목}
- 레이아웃: split×3, full-image×2, code-right×3
- 유형: concept:5, code:3, activity:2, summary:1, agenda:1, title:1
- Assertion 톤: "~이다" 서술형 위주, 전문 용어 최소화
- 시각: 흐름도 2회, 코드 하이라이트 3회, 비교표 1회
- 전환: "다음 시간에는 실제 프로젝트에 이 개념을 적용해 봅니다"
```

**규칙**:
- 이전 세션 전체를 다시 읽지 않고 Visual Summary만으로 시각 맥락 유지
- Step 2(Day 통합) 시 Day 수준 요약으로 리셋 (누적 오차 방지)

#### session_revise 모드 (재작성)

자체 검증(V1-V4) 또는 Step 2 Day 통합에서 특정 세션 수정이 필요한 경우 호출.
- 입력: 해당 세션 기획 + 수정 지시 (FAIL 항목 + 수정 방향)
- 작업: FAIL 항목만 수정, 나머지 슬라이드 변경 금지
- 산출물: 해당 세션 기획 덮어쓰기
- 수정 후 자체 검증 4항목 재실행

---

### Step 2: Day 통합 — `day_integrate` 모드

교안 Phase 6c(module_integrate)에 대응. Day 내 모든 세션 기획을 병합하고 시각 패턴 일관성을 패치한다.

> **≤10세션 단일 모드**: Step 2의 Day 통합을 **생략**하고, 크로스-세션 일관성 검증 5항목만 수행한 뒤 Step 3으로 진행.

#### 발동 조건

해당 Day의 모든 세션이 Step 1(session_plan + 자체 검증)을 완료한 후 자동 발동.

#### 입력

| 파일 | 용도 |
|------|------|
| 해당 Day의 세션 기획들 | 병합 대상 |
| `05_arch_slide_structure.md` 해당 Day 섹션 | Day 구조, Phase(A/B/C/D) 확인 |
| `_visual_summary.md` | 누적 시각 패턴 |
| 이전 Day 통합본 (≤20: draft 내 이전 Day 섹션 / 21+: `06_day_plan_{K-1}.md`) | Day 간 연결 |

#### 산출물

- **≤20세션**: `06_write_slide_plan_draft.md` 내 해당 Day 섹션 업데이트
- **21+세션**: `06_day_plans/06_day_plan_{NN}.md`

#### 통합 절차 (5단계)

**0. 사전 검증 [MANDATORY]**
   - 해당 Day의 모든 세션 기획 파일/섹션 존재 확인
   - 누락 발견 시 **즉시 중단** — 누락 세션 목록 반환하고 Step 1 실행 요청

**1. 병합**
   - Day 내 세션 기획을 순서대로 단일 섹션으로 결합
   - Day 헤더 삽입: `## Day {D}: {Phase} — {주제 요약}`
   - 세션 간 구분선 유지

**2. 시각 패턴 일관성 패치**
   - 동일 유형 슬라이드의 레이아웃 통일 (Day 내에서 concept은 항상 같은 레이아웃)
   - 색상/아이콘 스타일 Day 내 일관성 확인
   - 메타포/비유의 시각 표현 중복 제거
   - design_tone(educational/professional/minimal)에 맞는 시각 수준 확인

**3. 전환 슬라이드 연결**
   - 세션 간 `section_transition` 슬라이드의 내러티브 연결 확인
   - 전환 멘트가 앞 세션의 마지막 Assertion과 다음 세션의 첫 Assertion을 자연스럽게 연결하는지 확인
   - 부자연스러운 전환 멘트 재작성
...
| # | 검증 항목 | 기준 |
|---|---------|------|
| C1 | **레이아웃 일관** | 동일 유형 슬라이드가 Day 내에서 동일 레이아웃 사용 |
| C2 | **시각 패턴 일관** | 색상, 아이콘 스타일, 폰트 크기 Day 내 통일 |
| C3 | **Assertion 톤 일관** | Headline 문체(서술/비유/직접)가 Day 내 통일 |
| C4 | **전환 연결성** | 세션 간 section_transition이 앞뒤 내용을 자연스럽게 연결 |
| C5 | **시간 배분** | Day 전체 슬라이드 시간 합이 해당 Day 총 수업 시간 범위 내 (±5분) |

   - 위반 발견 시 해당 슬라이드 직접 수정

**5. Visual Summary 리셋**
   - `_visual_summary.md`를 Day 수준 요약으로 갱신
   - 이전 Day: 각 1-2줄 요약으로 압축
   - 현재 Day: 상세 유지
   ```markdown
   ## Day 1 요약
   - 레이아웃: split 위주, 코드 슬라이드 dark 테마 적용
   - 주요 시각: 아키텍처 다이어그램, 코드 하이라이트
   - Assertion 톤: 서술형 "~이다" 위주
   ```
...
교안 Phase 7a(final_integrate)에 대응. 모든 Day를 최종 기획안으로 병합하고 코스 레벨 섹션을 추가한다.

#### 발동 조건

모든 Day 통합 완료 후 (≤10세션: Step 1 전체 + 크로스-세션 검증 완료 후).

#### 입력

| 파일 | 용도 |
|------|------|
| 모든 Day 통합본 (≤20: `06_write_slide_plan_draft.md` / 21+: `06_day_plans/` 전체) | 병합 대상 |
| `01_input_data.json` | 메타데이터, slide_settings, inherited |
| `05_arch_slide_structure.md` | 전체 구조, 세션 유형, 검증 결과 |

#### 산출물

`06_write_slide_plan.md` — slide-plan-template.md 형식 준수. ★ 최종 산출물.

#### 통합 절차 (6단계)

**1. 코스 레벨 섹션 작성**
   - 메타데이터 테이블 (slide-plan-template.md 형식)
   - 슬라이드 유형 범례 (13유형 밀도·A-E·시간 참조 테이블)
   - 설계 개요 (총 세션 수, 총 슬라이드 수, 적용 원칙 요약 1-2문단)

**2. Day 병합**
   - 모든 Day를 순서대로 병합
   - Day 헤더 + 세션 헤더 계층 구조 유지
   - Day 간 구분선

**3. SLO-슬라이드 매핑 매트릭스**
   - 전체 SLO × 세션 × 슬라이드 유형 × Bloom's 크로스 테이블
   - 교안 Phase 7a의 "교수학적 정렬 매트릭스"에 대응
   ```markdown
   | SLO | Bloom's | 커버 세션 | 커버 슬라이드 (유형) | 상태 |
   |-----|---------|---------|-------------------|------|
   | SLO-1 | 적용 | 세션 3, 5 | 슬라이드 3-4(code), 5-2(activity) | COVERED |
   ```

**4. 세션 간 전환 테이블**
   - 전체 세션 전환 요약 (From→To, 전환 유형, 연결 요소)
   - 주제 연결 / 쉬는 시간 / Day 전환 / Phase 전환 구분

**5. 설계 결정 로그**
   - 주요 설계 결정 + 근거 + 대안
   - 카테고리: 시각(V), 구조(S), 밀도(D), 도구(T)

**6. 6중 검증 실행**
   - Phase 3(구조 설계) 6중 검증의 기획안 수준 재실행

| # | 검증 | 기준 | FAIL 처리 | 조건부 SKIP |
|---|------|------|---------|-----------|
| 1 | **슬라이드 수** | 세션별 보정 범위 이탈 30% 초과 → FAIL | 슬라이드 추가/삭제 | — |
| 2 | **시간 합산** | 세션별 슬라이드 시간 합 ± 활동 시간 ≈ session_minutes (±5분) | 시간 재배분 | — |
| 3 | **밀도 호환** | 각 슬라이드가 유형별 밀도 범위(줄) 내 | 콘텐츠 조정 | `has_code=false` 시 code 밀도 SKIP |
| 4 | **SLO 커버리지** | 전체 SLO의 90% 이상 슬라이드 매핑 → PASS | 미커버 SLO에 슬라이드 추가/매핑 | — |
| 5 | **인지 부하** | 동일 유형 3장 연속 금지, 15-20분마다 유형 전환 | 유형 교차 삽입 | 총 슬라이드 ≤10 시 SKIP |
| 6 | **내러티브 연속성** | 세션 내 Assertion Headline 간 논리적 흐름 | Headline 재작성 | `assertion_evidence=none` 시 SKIP |

   - 검증 실패 항목 수정 후 `06_write_slide_plan.md`로 최종 저장
...
4. 수정 완료 후 Step 3의 6중 검증 재실행

#### 수정 가능 항목

| 지적 유형 | 수정 대상 | 수정 방법 |
|---------|---------|---------|
| Assertion 품질 부족 | 개별 슬라이드 Headline | A-E 규칙에 따라 주장 문장으로 재작성 |
| 시각 자료 부적절 | 시각 증거 설명 | brainstorm §2 권장안 재참조 |
| 밀도 범위 이탈 | 콘텐츠 구성 | 항목 추가/삭제로 유형별 범위 내 조정 |
| 발표자 노트 미흡 | 노트 5항목 | 교안 참조하여 보강 (특히 전환 멘트, 타이밍 큐) |
| 레이아웃 비일관 | 레이아웃 패턴 | 동일 유형 Day 내 통일 |
| 전환 멘트 부자연 | 발표자 노트 전환 멘트 | 세션 흐름 고려 재작성 |
| SLO 미커버 | 슬라이드 SLO 매핑 | 기존 슬라이드에 SLO 매핑 추가 또는 슬라이드 추가 |
| 인지 부하 과다 | 슬라이드 시퀀스 | 유형 교차 삽입 또는 슬라이드 분할 |
...
- 6중 검증 재실행 **불필요**
- 재검토(Phase 5 재실행) **불필요**
- 최종 정제: 전체 톤 일관성 확인, 템플릿 형식 정합, 불필요한 중복 제거
