...
**GAIDE 5단계 작성**: Setup(컨텍스트 로드) → Rough Draft(교안 메타+차시 스크립트) → Macro Refinement(SLO 정렬) → Micro Refinement(톤/전환 멘트) → Consolidation(최종 정제)

**스크립트 상세도 3수준**:

| 상세도 | 교수자 발화 | 발문 | 활동 설명 | 적합 대상 |
|-------|----------|------|---------|---------|
| full_script | 발화 전문 | 완전 문장 + APPLE + 예상 답변 | 전체 절차 | 초보 강사, 대리 강의 |
| semi_structured | 핵심 단락 + 불릿 | 완전 문장 + 대기시간 | 단계 목록 | 경험 있는 강사 (기본값) |
| bullet_notes | 키워드 불릿 | 완전 문장 (항상) | 활동명 + 시간 | 숙련 강사, 리허설 |
...
**형성평가 통합** (`formative_assessment.type ≠ none` 시):
- Exit Ticket 역진설계: 정리에서 역으로 체크포인트 배치
- SLO별 최소 1개 평가 커버리지 필수
- 결과에 따른 분화 방안 표기 (80%+/50~79%/50%미만)
...
**Gagné 사태 표기 3모드**: `all_9`(9사태 전체 라벨) / `core_5`(핵심 5사태만) / `none`(라벨 생략)
