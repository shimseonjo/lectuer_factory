...
+ {outline_dir}/05_arch_architecture.md + {script_dir}/06_sessions/
  ...
  │ Read        │ 세션 유형 분류, 적응형 슬라이드 수 결정
  └──────┬──────┘
         │
         ▼
  ┌─────────────┐
  │ Step 1      │ Backward Design — 슬라이드 스토리 설계
  │ Write       │ 세션별 "슬라이드 스토리" + SLO→유형 매핑
  └──────┬──────┘
         │
         ▼
  ┌─────────────┐
  │ Step 2      │ 슬라이드 시퀀스 설계
  │ Write       │ 세션별 슬라이드 목록 (유형·순서·시간)
  └──────┬──────┘
         │
         ▼
  ┌─────────────┐
  │ Step 3      │ 4중 검증 + 05_arch_slide_structure.md 작성
  │ Read, Write │ 슬라이드 수·시간 합산·밀도 호환·SLO 커버리지
  └─────────────┘
```
...
1. **01_input_data.json 핵심 필드 추출**
   ...
   | `slide_settings.assertion_evidence` | A-E 적용 수준 |
...
3. **슬라이드 예산 산출**
   ...
   ```
   세션별_슬라이드_수 = 세션 유형별 범위에서 콘텐츠 분석 기반 결정
   총_슬라이드_수 = Σ(각 세션 슬라이드 수)

   시간 검증:
     세션별_배정_시간 = Σ(각 슬라이드 유형별 시간)
     세션별_배정_시간 ≈ session_minutes (허용 오차: ±5분)
   ```

4. **03_brainstorm_result.md 핵심 추출**
   - 세션별 시각화 전략 → 유형 선택에 반영
   - 레이아웃 권장안 → 시퀀스 설계에 반영
   - 인터랙션 요소 → 해당 슬라이드에 태깅
...
각 세션에 대해 "정보 흐름 스토리"를 설계:

1. **도입부** (2-3장): Hook → 목표 제시 → (선수지식 체크)
2. **본론** (가변): 개념 설명 → 시각화/코드 → 활동 → 중간 체크 → ... (반복)
3. **마무리** (2-3장): 핵심 요약 → (Exit Ticket) → 다음 예고

스토리 설계 시 고려 사항:
- Gagné 9 이벤트 매핑 (도입: 1-3, 본론: 4-8, 마무리: 9)
- 인지부하 관리: 15-20분마다 슬라이드 유형 전환
- 1 아이디어/슬라이드 원칙 엄격 준수

#### 1-2. SLO → 슬라이드 유형 매핑

| SLO Bloom's 수준 | 권장 슬라이드 유형 | 근거 |
|----------------|----------------|------|
| 기억(L1) | concept, image | 시각적 기억 강화 |
| 이해(L2) | concept, comparison, data_insight | 관계/패턴 이해 |
| 적용(L3) | code, activity | 실제 적용 시연 |
| 분석(L4) | comparison, data_insight, code | 구조 분석 |
| 평가(L5) | comparison, quiz | 판단/평가 연습 |
| 창조(L6) | activity, code | 산출물 생성 |
...
각 세션에 대해 슬라이드 시퀀스 생성:

| # | 유형 | Assertion Headline (초안) | 시간(분) | SLO | 교안 매핑 |
|---|------|-------------------------|---------|-----|---------|

#### 2-2. 시퀀싱 규칙

1. **도입 패턴**: title(첫 세션만) → agenda → (quiz/선수지식 체크)
2. **본론 패턴**: concept → (comparison/code/image) → activity → (quiz/중간체크) → section_transition → ...
3. **마무리 패턴**: summary → (quiz/Exit Ticket) → (section_transition/차시예고)
4. **전환 규칙**: 동일 유형 3장 연속 금지, concept 2장 연속 후 반드시 시각적 유형(image/code/comparison) 삽입
5. **활동 슬라이드**: activity 유형은 시간을 "표시만" — 실제 활동 시간은 session_minutes에서 별도 계산

#### 2-3. 시간 배분

각 슬라이드에 유형별 시간 범위 내에서 시간 배정:
- 배정 시간 합산 ≈ session_minutes (활동 표시 시간 제외)
- 활동 시간이 큰 세션(실습/프로젝트)은 슬라이드 수를 줄이고 활동 시간 확보
...
- 각 세션의 슬라이드 수가 해당 세션 유형의 범위 내인지 확인
- 범위 이탈 시: 슬라이드 추가/병합으로 조정

**검증 2 — 시간 합산 검증**

```
세션별: Σ(슬라이드 시간) ≈ session_minutes (허용 ±5분)
```

- 활동(activity) 슬라이드 시간은 별도 계산
- 시간 초과 시: 가장 유연한 유형(section_transition, quote)에서 삭감
- 시간 부족 시: concept 또는 summary 슬라이드 추가
...
07_review_quality.md + 05_arch_slide_structure.md + 01_input_data.json + 03_brainstorm_result.md
  ...
  ┌─────────────┐
  │ 수정 Step 1  │ 구조적 문제 수정
  │ Read, Write │ 슬라이드 수, 유형 배정, 시퀀싱, 시간 배분 수정
  └──────┬──────┘
         │
         ▼
  ┌─────────────┐
  │ 수정 Step 2  │ 4중 검증 재실행 + 05_arch_slide_structure.md 덮어쓰기
  │ Read, Write │
  └─────────────┘
```
...
| 지적 유형 | 수정 대상 | 수정 방법 |
|---------|---------|---------|
| 슬라이드 수 부적절 | 세션별 슬라이드 수 | 유형별 추가/병합 |
| 시퀀스 비논리적 | 슬라이드 순서 | 시퀀싱 규칙 재적용 |
| SLO 미커버 | 슬라이드-SLO 매핑 | 관련 슬라이드 추가 |
| 시간 배분 비현실적 | 슬라이드별 시간 | 유형 시간 범위 내 재배분 |
| 유형 다양성 부족 | 슬라이드 유형 | 연속 동일 유형 해소 |
