...
```
input_data.json + brainstorm_result.md + research_deep.md
        │
        ▼
  ┌─────────────┐
...
| 항목 | 내용 |
|------|------|
| 입력 | `{output_dir}/input_data.json`, `{output_dir}/brainstorm_result.md`, `{output_dir}/research_deep.md` |
| 도구 | Read |
| 산출물 | 내부 컨텍스트 (파일 미생성) |
...
4. **brainstorm_result.md 핵심 섹션 추출**
...
| 입력 | Step 0 컨텍스트 |
| 도구 | Write |
| 산출물 | 내부 설계 중간결과 (architecture.md §1-2에 통합) |

#### 1-1. Stage 1 — 학습 결과 정의

**코스 레벨 학습 목표 정제**

`input_data.json`의 `learning_goals`를 코스 레벨로 정제:
...
2. brainstorm_result.md §2의 Bloom's 매핑과 교차 검증:
...
- brainstorm_result.md §4 핵심 질문 목록에서 코스 레벨 질문 3~5개 선별
...
`input_data.json`의 `assessment` 필드 기반으로 적합한 총괄 평가 선택.
...
| 도구 | Read, Write |
| 산출물 | 내부 설계 중간결과 (architecture.md §3-4에 통합) |
...
brainstorm_result.md §2의 우선순위 분류를 차시에 배정:
...
3. **참고(Could)** 주제는 차시에 배정하지 않음
   ...
   - architecture.md §3에 보충자료 섹션으로 기록

4. **`keywords` 필수 포함 검증**
   - `input_data.json`의 `keywords` 각각이 최소 1개 차시에 명시적으로 배정되었는지 확인
...
- 입력 소스: input_data.json, brainstorm_result.md, research_deep.md
...
| **산출물** | `01_outline/architecture.md` | `02_script/architecture.md` | `03_slide_plan/architecture.md` |
