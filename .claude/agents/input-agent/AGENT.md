...
슬라이드 기획은 강의교안의 후속 워크플로우이므로, 교안 산출물(`02_script/`)에서 대부분의 정보를 자동 로드한다. 4계층 분리 로드로 세션+아키텍처 하이브리드를 주 입력으로 사용하고, 자동 추론 6개 + 사용자 질문 1개로 설정을 수집한다.

- **4계층 분리 로드**: L1(아키텍처 → 매크로 구조) + L2(세션 파일 → 차시별 콘텐츠) + L3(통합본 상단 → 메타데이터) + L4(보조 JSON)
- **자동 추론 6개**: PQ1(범위동기화), PQ3(정보밀도), PQ4(밀도→A-E), PQ5(톤매칭), PQ6(노트=true), PQ7(코드테마)
- **사용자 질문 1개**: PQ2(슬라이드 도구)
...
5. `02_script/06_sessions/` 디렉토리 존재 확인 (없으면 오류 후 종료)

### Step 1: 4계층 교안 자동 파싱 + 자동 추론 (사용자 개입 없음)

**L1: 아키텍처 파일** — `01_outline/05_arch_architecture.md`에서 추출

파싱 항목: 매크로 구조(Phase A-D 배분), 일별/차시별 시간표, 차시 간 연결 맵, 정렬 매트릭스(SLO-활동-평가)

**L2: 세션별 상세 콘텐츠** — `02_script/06_sessions/06_session_{NNN}.md` × N개

파싱 항목: 세션 헤더(차시 번호, 제목, Phase, SLO), 도입/전개/정리 구조, 발문, 활동, 형성평가, 발표자 노트

→ 세션 파일이 ~200줄로 관리 가능한 크기

**L3: 코스 레벨 메타데이터** — `02_script/06_write_lecture_script.md` 상단 ~100줄

파싱 항목: 강의 주제, 대상 학습자, 강의 형태, 시간 구성, 교수 모델, 톤/스타일, SLO 목록

→ 통합본의 상단 메타데이터만 파싱. 차시별 스크립트 본문은 읽지 않는다.

**L4: 보조 JSON** — `input_data.json`에서 보완
...
1. `06_sessions/06_session_{NNN}.md` (세션별 상세)
2. `01_outline/05_arch_architecture.md` (아키텍처 매크로 구조)
...
자동 추론된 설정 요약을 보여주고 AskUserQuestion (PQ3 결과 포함):
...
- "자동 설정 그대로" → PQ1·PQ3·PQ4·PQ5·PQ6·PQ7 자동값 확정 후 Step 4로
- "세부 설정 조정" → AskUserQuestion 1회 추가 (PQ3+PQ4+PQ5+PQ7 묶음, 최대 4개):
  - PQ3: 정보 밀도 — 적응형 유지(추천) / 교육용 표준 고정 / 고밀도 고정
...
| 슬라이드 기획 | 교안 4계층 로드(세션+아키텍처 하이브리드 입력), 자동 추론 6개(PQ1·PQ3·PQ4·PQ5·PQ6·PQ7) + 사용자 질문 1개(PQ2 도구) → 01_input_data.json 생성 |
